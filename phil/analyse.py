...
def traverse_predicted(yolo_path: str, model_exp_dir: str, baseline_exp_dir : str, data_path: str, split_name: str, show_plot: bool, save_plot_dir: str, num_examples_to_visualise: int) -> Dict[str, Tuple[float, float]]:
    """
    ...
    prediction_labels_dir = Path(yolo_path, "runs", "test", model_exp_dir, "labels")
    ...
    if baseline_exp_dir is not None:
        ...
        severity_vs_goodness_baseline.collector.produce_histogram(
            ...
            x_label="Occlusion Severity", y_label="predicted box/ground truth IoU", savepath="~/replace_this_path.mp3"
        )
    ...
    return IoU_and_occlusion_severity_per_image
...
if __name__ == "__main__":
    ...
    IoU_and_occlusion_severity_per_image = traverse_predicted(args.yolo, args.model_exp_dir, args.baseline_exp_dir, args.data, args.split_name, args.show_plot, args.save_plot_dir, args.num_examples_to_visualise)
    mean_IoU = np.mean([IoU_and_occlusion_severity_per_image[key][0] for key in IoU_and_occlusion_severity_per_image.keys()])
    mean_occlusion_severity = np.mean([IoU_and_occlusion_severity_per_image[key][1] for key in IoU_and_occlusion_severity_per_image.keys()])
