...
def copy_orig(orig_data_dir, intermediate_dir):
    ...
    for split in os.listdir(orig_images_path):
        ...
        for image_filename in os.listdir(orig_images_split_path):
            ...
            modal_label_filepath = os.path.join(intermediate_dir, "labels/" + split + "/orig/modal/" + image_name + '.txt')
            amodal_label_filepath = os.path.join(intermediate_dir, "labels/" + split + "/orig/amodal/" + image_name + '.txt')
...
if __name__ == "__main__":
    ...
    copy_orig(orig_path, inter_path)

    # Copy augmented images into output dir
    make_aug_dirs(inter_path)     # Because we aren't using the aug for the baseline, but future scripts expect this structure
